...
//					System.out.println(this.bQuer.getVec()[i].doubleValue());
					...
					for(int j = 0; j < this.nichtbasis.length; j++){
						if(nichtbasis[j] < indexOfKuenstlicheVar){
							if(basisInverse.multiplyRowColumn(m, i, nichtbasis[j]).signum() != 0){
//								System.out.println("BasisInv: "+basisInverse);
...
//		System.out.println(nichtbasis.length);
		for( int i=0 ; i<nichtbasis.length ; i++){
			Fraction redCost;
			...
			if(this.isBlandOn){
				if(redCost.signum() > 0 && nichtbasis[i] < MaxMinIndex){//Kleinster-Variablen-Index-Regel
					MaxIndex = i;
...
//				if( redCost.doubleValue()>max.doubleValue()){//Steilster-Anstieg-Regel
				if( redCost.compareTo(max)>0){//Steilster-Anstieg-Regel
					MaxIndex = i;	
					max = redCost;
				}
			}
...
//			return null;
	...
	private Tupel<Integer, Fraction> lambda0(Vector d){
		Fraction minLambda= null;
		int index = -1;
		int MinIndex=0;
		for(int i = 0; i < this.bQuer.getVec().length; i++){
			if((minLambda==null && d.get(i).signum() > 0)){
				minLambda = this.bQuer.get(i).divide( d.get(i) );
				index = i;
				MinIndex = basis[i];
			} else if(minLambda!=null && d.get(i).signum() > 0){
				if((minLambda.compareTo(this.bQuer.get(i).divide(d.get(i)))>0 && d.get(i).signum() > 0)){
					minLambda = this.bQuer.get(i).divide( d.get(i) );
					index = i;
					MinIndex = basis[i];
				}
				else if(minLambda.compareTo(this.bQuer.get(i).divide(d.get(i)))==0 && d.get(i).signum() > 0){//Kleinster-Variablen-Index-Regel
					if(MinIndex > basis[i]){
						MinIndex = basis[i];
						index = i;
					}
